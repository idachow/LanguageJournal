...
class Window(object):
	def __init__(self,canvas,width,height,cx,cy,root):
		...
		# setting custom universal color palette
	...
	def button_all_back(self): 
		# graphics for back button
		y = self.height/20
		x = self.height/20
		self.canvas.create_rectangle(x,y,x+100,y+100,fill=self.color_darkergray)
		self.canvas.create_text(x+50,y+50,text="<--",font="Calibri 36 bold",
								fill=self.color_lightteal)


	def button_start_plus(self):
		# graphics for add entry button (from start page)
		cx, cy = self.cx, self.cy
		self.canvas.create_text(cx,cy-(.25*self.height),text="Language Journal", 
								font="Calibri 24 bold",fill=self.color_offwhite)
		self.canvas.create_rectangle(cx+10,cy+50,cx+110,cy-50,fill=self.color_darkergray)
		self.canvas.create_text(cx+60,cy,text="+",font="Calibri 36 bold",
								fill=self.color_lightteal)

	def button_start_view(self):
		# graphics for view all button (from start page)
		cx, cy = self.cx, self.cy
		...
		self.canvas.create_rectangle(cx-10,cy+50,cx-110,cy-50,fill=self.color_darkergray)
		# three "lines"
		self.canvas.create_text(cx-60,cy-50,text="___",font="Calibri 30 bold",
								fill=self.color_lightteal)
		self.canvas.create_text(cx-60,cy-20,text="___",font="Calibri 30 bold",
								fill=self.color_lightteal)
		self.canvas.create_text(cx-60,cy+10,text="___",font="Calibri 30 bold",
								fill=self.color_lightteal)
	...
	def updateFileParameters(self):
		# reads through file and udpates with last fileparams
		# this helps with the bug where you can't update two things at the same time
		# since it'll save via the old filename/filedef
	...
	def screen_viewVocab_scroller(self,event):
		# scrollbox frame settings
		cx, cy = self.cx, self.cy
		self.canvasScroller.configure(scrollregion=self.canvasScroller.bbox("all"),
									width=125,height=cy)


	def screen_viewVocab_displayVocab(self):
		# screen layout for displaying vocab
			...
			with open('data.csv', 'rb') as csvfile:
				...
				for row in filereader:
					if r == voc:
						...
						self.filedef = row[2]
						self.filedate = row[0]
						self.canvas.create_text(cx,cy-cy/2,text=self.filename,
		...
		self.b5 = Button(self.frame_displayaudioplay,text="Play Recorded Audio", 
						command=lambda:self.interPlayAudio())
		self.b5.pack()
		...
		self.b6 = Button(self.frame_displayeditvoc,text="EDIT VOCABULARY", 
						command=lambda:self.screen_editVocab())
		self.b6.pack()
	...
	def interPlayAudio(self):
		# intermediate function due to bug
		self.playAudio(self.filename)

	def screen_viewVocab(self):
		# default main screen for view vocab
		width = self.width
		height = self.height
		...
		self.myscrollbar=Scrollbar(self.myframe,orient="vertical",
									command=self.canvasScroller.yview)
		self.canvasScroller.configure(yscrollcommand=self.myscrollbar.set)

		# calls into other scroller function to allow proper scrolling
	...
	def deleteRow(self):
		# deletes row in csv by creating new outdata.csv file temporarily
		# creates outdata.csv
		open("outdata.csv", 'a').close()

		FIRST_ROW_NUM = 0
		rows_to_delete = {self.filerow}

		with open('data.csv', 'rt') as infile, open('outdata.csv', 'wt') as outfile:
		...
		dst = "data.csv"
		os.rename(src,dst)
		# ... and then copies it back over
		os.remove("audio/" + self.filename + ".wav")



	def edit_audio(self):
		# remove the audio (all files come with either 
		# essentially empty .wav file or a 3 second long one)
		self.updateFileParameters()
		os.remove("audio/" + self.filename + ".wav")
		self.set_audio(self.filename,3)

	def edit_audioINIT(self):
		# applies only to the add vocab function
		os.remove("audio/" + self.lastadded + ".wav")
		self.set_audio(self.lastadded,3)	
	...
	def screen_editVocab(self):
		...
		self.canvas.create_text(cx,cy-cy/2,text="Editing:",font="Calibri 24 bold",
									fill=self.color_offwhite,anchor="w")
		self.canvas.create_text(cx,cy-cy/2+50,text="'" + self.filename + "'",
								font="Calibri 24 bold",
									fill=self.color_lightorange,anchor="w")
		...
		def edit_word():
			...
			# save properly into class
			self.editedWord = Vocab(str(fetche), " ", " ")
			self.editedWord.editWord(fetche,self.filerow,self.filedate,
									self.filedef,self.filename)
		...
		def edit_return():
			# more destroying... returning back but have to clear frames
			self.canvas.create_rectangle(cx-25,cy-cy/2-20,self.width,self.height,
									fill=self.color_bggray,outline=self.color_bggray)
			self.frame_editTermSave.destroy()
			self.frame_editTerm.destroy()
		...
		self.canvas.create_text(cx,cy-125,text="Change Vocabulary Term:", 
								fill="white", anchor="w")
		...
		# frame for vocab edit entry
		self.canvas.create_text(cx,cy,text="Change Vocabulary Definition:", 
								fill="white", anchor="w")
		...
		self.bSaveAudio = Button(self.frame_editAudioSave,text="o Re-record",
								command=lambda:self.edit_audio())
		self.bSaveAudio.pack()
		self.bSaveReplay = Button(self.frame_editAudioReplay,
								text="> Replay re-recording",
								command=lambda:self.playAudio(self.filename))
		self.bSaveReplay.pack()




		self.frame_editDelete=Frame(self.root,relief=GROOVE,width=0,height=0,bd=0)
		self.frame_editDelete.place(x=cx+200,y=cy+250)

		self.bDelete = Button(self.frame_editDelete,text="x DELETE ENTRY",
	...
	def screen_newVocab(self):
		...
		cx, cy = self.cx, self.cy
		self.canvas.create_text(cx,50,text="Create new entry",
							font="Calibri 36 bold",fill=self.color_lightteal)
		def set_text():
			fetche = self.e.get()
			fetche2 = self.e2.get()
		...
		self.e.pack()
		self.e2.pack()
		# buttons all for save / record audio
		self.b1 = Button(self.frame_entrysave,text="Save",command=lambda:set_text())
		self.b2 = Button(self.frame_audiosave,text="o Record audio",
						command=lambda:self.edit_audioINIT())
