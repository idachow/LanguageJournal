...
class Window(object):
	...
	def screen_viewVocab_scroller(self,event):
		cx, cy = self.cx, self.cy
		self.canvasScroller.configure(scrollregion=self.canvasScroller.bbox("all"),width=cx-cx/3,height=cy)
	...
	def screen_viewVocab(self):
		...
		cx, cy = self.cx, self.cy
		self.canvas.create_text(cx,50,text="view all vocab",font="Calibri 36 bold",fill=self.color_lightteal)
